...
import { dailyDigestSchema, type DailyDigest } from "@/lib/reports/schemas"; // this brings in the Zod schema and the inferred TypeScript type
...
export async function generateDailyDigest(
  ...
  const { report, model, inputTokens, outputTokens } = await generateReportContent({ // this calls Gemini (retrying once on transient overload), falling back to Groq only if Gemini fails entirely
    schema: dailyDigestSchema, // this is the real schema both engines' output is validated against
